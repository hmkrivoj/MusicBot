...
public final class FinderUtil {
   ...
   * net.dv8tion.jda.api.entities.User User}s.
   *
   * <p>If a {@link net.dv8tion.jda.api.sharding.ShardManager ShardManager} is available this will
   * query across that instead of the JDA instance.
   *
   ...
   * @return A possibly-empty {@link java.util.List List} of Users found by the query from the
   *     provided JDA instance.
   */
   ...
   * net.dv8tion.jda.api.entities.User User}s.
   *
   * <p>This only queries the instance of JDA, regardless of whether or not a {@link
  ...
  private static List<User> jdaUserSearch(String query, JDA jda, boolean useShardManager) {
    ...
    if (userMention.matches()) {
      User user =
          manager != null
              ? manager.getUserById(userMention.group(1))
      ...
      List<User> users =
          (manager != null ? manager.getUserCache() : jda.getUserCache())
              .stream()
                  .filter(
                      user ->
                          user.getName().toLowerCase().equals(lowerName)
    ...
    if (!exact.isEmpty()) return Collections.unmodifiableList(exact);
    if (!wrongcase.isEmpty()) return Collections.unmodifiableList(wrongcase);
    if (!startswith.isEmpty()) return Collections.unmodifiableList(startswith);
   ...
   * <p><b>WARNING</b>
   *
   * <p>Unlike the other finder methods, this one has two very unique features that set it apart
   * from the rest:
   *
   * <ul>
   *   <li><b>1)</b> In order to get a list of bans that is usable, this method initial retrieves it
   *       by usage of {@link net.dv8tion.jda.api.requests.RestAction#complete()
   *       Guild#getBans().complete()}. Because of this, as would be the same expected effect from
   *       the other utility methods, this will block the thread it is called in. The difference,
   *       however, comes in that this method may have slight variations in return speed, especially
   *       when put under higher usage over a shorter period of time.
   *   <li><b>2) This method can return {@code null}</b> if and only if an {@link
   *       java.lang.Exception Exception} is thrown while initially getting banned Users via {@link
   *       net.dv8tion.jda.api.entities.Guild#retrieveBanList() Guild#getBanList()}.
   * </ul>
   *
   * @param query The String query to search by
   * @param guild The Guild to search for banned Users from
   * @return A possibly-empty {@link java.util.List List} of Users found by the query from the
   *     provided JDA instance, or {@code null} if an {@link java.lang.Exception Exception} is
   *     thrown while initially getting banned Users.
  ...
  public static List<User> findBannedUsers(String query, Guild guild) {
    ...
    try {
      bans =
          guild.retrieveBanList().complete().stream()
    ...
    if (userMention.matches()) {
      ...
      if (user != null && bans.contains(user)) return Collections.singletonList(user);
      for (User u : bans) if (u.getId().equals(id)) return Collections.singletonList(u);
      ...
      if (user != null && bans.contains(user)) return Collections.singletonList(user);
      for (User u : bans) if (u.getId().equals(query)) return Collections.singletonList(u);
    }
    ArrayList<User> exact = new ArrayList<>();
    ArrayList<User> wrongcase = new ArrayList<>();
    ArrayList<User> startswith = new ArrayList<>();
    ...
    guild
        ...
        .forEach(
            member -> {
              ...
              if (name.equals(query) || effName.equals(query)) exact.add(member);
  ...
  private static List<TextChannel> jdaTextChannelSearch(
      String query, JDA jda, boolean useShardManager) {
    Matcher channelMention = CHANNEL_MENTION.matcher(query);
    ...
    if (channelMention.matches()) {
      TextChannel tc =
          manager != null
              ? manager.getTextChannelById(channelMention.group(1))
              : jda.getTextChannelById(channelMention.group(1));
      if (tc != null) return Collections.singletonList(tc);
    } else if (DISCORD_ID.matcher(query).matches()) {
      TextChannel tc =
          manager != null ? manager.getTextChannelById(query) : jda.getTextChannelById(query);
      if (tc != null) return Collections.singletonList(tc);
    }

    return genericTextChannelSearch(
        query, manager != null ? manager.getTextChannelCache() : jda.getTextChannelCache());
  }
  ...
  private static List<VoiceChannel> jdaVoiceChannelSearch(
      String query, JDA jda, boolean useShardManager) {
    ShardManager manager = useShardManager ? jda.getShardManager() : null;

    if (DISCORD_ID.matcher(query).matches()) {
      VoiceChannel vc =
          manager != null ? manager.getVoiceChannelById(query) : jda.getVoiceChannelById(query);
      if (vc != null) return Collections.singletonList(vc);
    }

    return genericVoiceChannelSearch(
        query, manager != null ? manager.getVoiceChannelCache() : jda.getVoiceChannelCache());
  }
   ...
   * @param query The String query to search by
   * @param jda The instance of JDA to search from
   * @return A possibly-empty {@link java.util.List List} of Categories found by the query from the
   *     provided JDA instance.
   */
  ...
  private static List<Category> genericCategorySearch(
      String query, SnowflakeCacheView<Category> cache) {
   ...
   *
   * <ul>
   *   <li>Emote Mention: Query provided matches a :emote: mention (more specifically {@literal
   *       <:emoteName:emoteID>}). <br>
   *       Note: This only returns here if the emote is <b>valid</b>. Validity being the ID
   *       retrieves a non-null Emote and that the {@link
   *       net.dv8tion.jda.api.entities.Emote#getName() name} of the Emote is equal to the name
   *       found in the query.
   * </ul>
   *
   * @param query The String query to search by
   * @param jda The instance of JDA to search from
   * @return A possibly-empty {@link java.util.List List} of Emotes found by the query from the
   *     provided JDA instance.
   */
