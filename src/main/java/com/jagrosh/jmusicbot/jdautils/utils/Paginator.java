 ...
 *
 * <ul>
 *   <li><b>Left Arrow</b> - Causes the Paginator to traverse one page backwards.
 *   <li><b>Stop</b> - Stops the Paginator.
 *   <li><b>Right Arrow</b> - Causes the Paginator to traverse one page forwards.
 * </ul>
 *
 * Additionally, if specified in the {@link Paginator.Builder}, two "bulk skip" reactions will be
 * added to allow a certain number of pages to be skipped left or right. <br>
 * Paginator.Builders can also set a Paginator to accept various forms of text-input, such as left
 * and right text commands, and even user specified page number via text.
 *
 * @author John Grosh
 */
public class Paginator extends Menu {
  ...
  private void pagination(Message message, int pageNum) {
    ...
    else paginationWithoutTextInput(message, pageNum);
  }
        ...
        event -> {
          if (event instanceof MessageReactionAddEvent messageReactionAddEvent) {
            ...
            if (rawContent.equalsIgnoreCase(leftText) && (1 < pageNum || wrapPageEnds))
              ...
              // logic
              // when checking the event in the block above this action block has guaranteed this is
              // the only
            ...
            mre.getMessage()
                .delete()
                .queue(v -> {}, t -> {}); // delete the calling message so it doesn't get spammy
          }
        },
        timeout,
        unit,
  ...
  private void handleMessageReactionAddAction(
      MessageReactionAddEvent event, Message message, int pageNum) {
  ...
  public static class Builder extends Menu.Builder<Builder, Paginator> {
     ...
     * relative to the total page number and the current page as determined by the provided {@link
     * java.util.function.BiFunction BiFunction}. <br>
     * As the page changes, the BiFunction will re-process the current page number and the total
     * page number, allowing for the displayed text of the Message to change depending on the page
     * number.
     *
     * @param textBiFunction The BiFunction that uses both current and total page numbers to get
