 ...
 * {@link OrderedMenu.Builder#setSelection(BiConsumer)} and {@link
 * OrderedMenu.Builder#setCancel(Consumer)} methods.
 *
 * @author John Grosh
 */
public class OrderedMenu extends Menu {
  ...
  OrderedMenu(
      ...
      String description,
      List<String> choices,
      BiConsumer<Message, Integer> action,
      Consumer<Message> cancel,
      boolean useLetters,
      boolean allowTypedInput,
      boolean useCancel) {
  ...
  /**
   * Shows the OrderedMenu as a new {@link net.dv8tion.jda.api.entities.Message Message} in the
   ...
   * @throws java.lang.IllegalArgumentException If <b>all</b> of the following are violated
   *     simultaneously:
   *     <ul>
   *       <li>Being sent to a {@link net.dv8tion.jda.api.entities.TextChannel TextChannel}.
   *       <li>This OrderedMenu does not allow typed input.
  ...
  /**
   * Displays this OrderedMenu by editing the provided {@link net.dv8tion.jda.api.entities.Message
   * Message}.
   *
        ...
        m -> {
          try {
            ...
            for (int i = 0; i < choices.size(); i++) {
              ...
              if (i < choices.size() - 1) m.addReaction(getEmoji(i)).queue();
              // If this is the last run of this loop
                ...
                re.queue(
                    v -> {
                      ...
                      if (allowTypedInput) waitGeneric(m);
                      else waitReactionOnly(m);
                    });
              }
            }
        ...
        e -> {
          ...
          if (e instanceof MessageReactionAddEvent messageReactionAddEvent) {
            ...
            if (event.getReaction().getReactionEmote().getName().equals(CANCEL)) cancel.accept(m);
              ...
              // The int provided in the success consumer is not indexed from 0 to number of choices
              // - 1,
              // but from 1 to number of choices. So the first choice will correspond to 1, the
              // second
              // choice to 2, etc.
  ...
  private boolean isValidReaction(Message m, MessageReactionAddEvent e) {
    ...
    if (!isValidUser(e.getUser(), e.isFromGuild() ? e.getGuild() : null)) return false;
  ...
  private boolean isValidMessage(Message m, MessageReceivedEvent e) {
    ...
    if (!e.getChannel().equals(m.getChannel())) return false;
  ...
  public static class Builder extends Menu.Builder<Builder, OrderedMenu> {
    ...
    /**
     * Sets the {@link java.util.function.BiConsumer BiConsumer} action to perform upon selecting a
     * option.
     *
