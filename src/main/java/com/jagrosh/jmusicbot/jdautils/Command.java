 ...
 * com.jagrosh.jmusicbot.jdautils.Command#execute(com.jagrosh.jmusicbot.jdautils.CommandEvent)
 ...
 * }</code></pre>
 *
 * Execution is with the provision of a MessageReceivedEvent-CommandClient wrapper called a {@link
 * com.jagrosh.jmusicbot.jdautils.CommandEvent CommandEvent} and is performed in two steps:
 *
 * <ul>
 *   <li>{@link com.jagrosh.jmusicbot.jdautils.Command#run(CommandEvent) run} - The command runs
 *       through a series of conditionals, automatically terminating the command instance if one is
 *       not met, and possibly providing an error response.
 *   <li>{@link com.jagrosh.jmusicbot.jdautils.Command#execute(CommandEvent) execute} - The command,
 *       now being cleared to run, executes and performs whatever lies in the abstract body method.
 * </ul>
 *
...
public abstract class Command {
  /**
   * The name of the command, allows the command to be called the format: {@code [prefix]<command
   * name>}.
   */
  ...
  /**
   * The {@link com.jagrosh.jmusicbot.jdautils.Command.Category Category} of the command. <br>
  ...
  /**
   * {@code true} if the command may only be used in a {@link net.dv8tion.jda.api.entities.Guild
   * Guild}, {@code false} if it may be used in both a Guild and a DM. <br>
   * Default {@code true}.
   */
  ...
  /**
   * {@code true} if the command may only be used by a User with an ID matching the Owners or any of
   * the CoOwners. <br>
   * Default {@code false}.
   */
  ...
  /**
   * The {@link com.jagrosh.jmusicbot.jdautils.Command.CooldownScope CooldownScope} of the command.
   * This defines how far of a scope cooldowns have. <br>
   * Default {@link com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#USER CooldownScope.USER}.
   */
  ...
  /**
   * The main body method of a {@link com.jagrosh.jmusicbot.jdautils.Command Command}. <br>
   * This is the "response" for a successful {@link
   * com.jagrosh.jmusicbot.jdautils.Command#run(CommandEvent) #run(CommandEvent)}.
   *
   * @param event The {@link com.jagrosh.jmusicbot.jdautils.CommandEvent CommandEvent} that
   *     triggered this Command
   */
  protected abstract void execute(CommandEvent event);

  /**
   * Runs checks for the {@link com.jagrosh.jmusicbot.jdautils.Command Command} with the given
   * {@link com.jagrosh.jmusicbot.jdautils.CommandEvent CommandEvent} that called it. <br>
      ...
      for (Permission p : botPermissions) {
        if (p.isChannel()) {
          if (p.name().startsWith("VOICE")) {
            ...
            if (vc == null) {
              ...
              terminate(
                  event,
                  String.format(
                      BOT_PERM, event.getClient().getError(), p.getName(), "Voice Channel"));
              return;
            }
          } else {
            if (!event.getSelfMember().hasPermission(event.getTextChannel(), p)) {
              terminate(
                  event,
                  String.format(BOT_PERM, event.getClient().getError(), p.getName(), "Channel"));
              return;
            }
          }
        } else {
          if (!event.getSelfMember().hasPermission(p)) {
            terminate(
                event, String.format(BOT_PERM, event.getClient().getError(), p.getName(), "Guild"));
            return;
          }
        }
      }
      ...
      for (Permission p : userPermissions) {
        if (p.isChannel()) {
          if (!event.getMember().hasPermission(event.getTextChannel(), p)) {
            terminate(
                event,
                String.format(USER_PERM, event.getClient().getError(), p.getName(), "Channel"));
            return;
          }
        } else {
          if (!event.getMember().hasPermission(p)) {
            terminate(
                event,
                String.format(USER_PERM, event.getClient().getError(), p.getName(), "Guild"));
            return;
          }
        }
      }
  ...
  /**
   * Checks whether a command is allowed in a {@link net.dv8tion.jda.api.entities.TextChannel
   * TextChannel} by searching the channel topic for topic tags relating to the command.
   *
   * <p>{-{@link com.jagrosh.jmusicbot.jdautils.Command#name name}}, {-{@link
   * com.jagrosh.jmusicbot.jdautils.Command.Category category name}}, or {-{@code all}} are valid
   * examples of ways that this method would return {@code false} if placed in a channel topic.
   *
   * <p><b>NOTE:</b>Topic tags are <b>case sensitive</b> and proper usage must be in lower case!
   * <br>
   * Also note that setting {@link com.jagrosh.jmusicbot.jdautils.Command#usesTopicTags
   * usesTopicTags} to {@code false} will cause this method to always return {@code true}, as the
   * feature would not be applicable in the first place.
   *
  ...
  /**
   * Gets the {@link com.jagrosh.jmusicbot.jdautils.Command#name Command.name} for the Command.
   *
  ...
  /**
   * Gets the {@link com.jagrosh.jmusicbot.jdautils.Command#help Command.help} for the Command.
   *
  ...
  /**
   * Gets the {@link com.jagrosh.jmusicbot.jdautils.Command#category Command.category} for the
   * Command.
   *
  ...
  /**
   * Gets the {@link com.jagrosh.jmusicbot.jdautils.Command#arguments Command.arguments} for the
   * Command.
   *
  ...
  /**
   * Gets the proper cooldown key for this Command under the provided {@link
  ...
  /**
   * Gets an error message for this Command under the provided {@link
   * com.jagrosh.jmusicbot.jdautils.CommandEvent CommanEvent}.
   *
  ...
  /**
   * To be used in {@link com.jagrosh.jmusicbot.jdautils.Command Command}s as a means of organizing
   * commands into "Categories" as well as terminate command usage when the calling {@link
   * com.jagrosh.jmusicbot.jdautils.CommandEvent CommandEvent} doesn't meet certain requirements.
   *
   * @author John Grosh (jagrosh)
   */
  public static class Category {
     ...
     *
     * @param event The {@link com.jagrosh.jmusicbot.jdautils.CommandEvent CommandEvent} that was
  ...
  /**
   * A series of {@link java.lang.Enum Enum}s used for defining the scope size for a {@link
   * com.jagrosh.jmusicbot.jdautils.Command Command}'s cooldown.
   *
   * <p>The purpose for these values is to allow easy, refined, and generally convenient keys for
   * cooldown scopes, allowing a command to remain on cooldown for more than just the user calling
   * it, with no unnecessary abstraction or developer input.
   *
   * <p>Cooldown keys are generated via {@link
   * com.jagrosh.jmusicbot.jdautils.Command#getCooldownKey(CommandEvent)
   * Command#getCooldownKey(CommandEvent)} using 1-2 Snowflake ID's corresponding to the name (IE:
   * {@code USER_CHANNEL} uses the ID's of the User and the Channel from the CommandEvent).
   *
   * <p>However, the issue with generalizing and generating like this is that the command may be
   * called in a non-guild environment, causing errors internally. <br>
   * To prevent this, all of the values that contain "{@code GUILD}" in their name default to their
   * "{@code CHANNEL}" counterparts when commands using them are called outside of a {@link
   * net.dv8tion.jda.api.entities.Guild Guild} environment.
   *
   * <ul>
   *   <li>{@link com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#GUILD GUILD} defaults to
   *       {@link com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#CHANNEL CHANNEL}.
   *   <li>{@link com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#USER_GUILD USER_GUILD}
   *       defaults to {@link com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#USER_CHANNEL
   *       USER_CHANNEL}.
   * </ul>
   *
   * These are effective across a single instance of JDA, and not multiple ones, save when multiple
   * shards run on a single JVM and under a {@link net.dv8tion.jda.api.sharding.ShardManager
   * ShardManager}. <br>
   * There is no shard magic, and no guarantees for a 100% "global" cooldown, unless all shards of
   * the bot run under the same ShardManager, and/or via some external system unrelated to
   * JDA-Utilities.
   *
   * @since 1.3
   * @author Kaidan Gustave
   * @see com.jagrosh.jmusicbot.jdautils.Command#cooldownScope Command.cooldownScope
   */
  public enum CooldownScope {
    ...
    /**
     * Applies the cooldown to the {@link net.dv8tion.jda.api.entities.MessageChannel
    ...
    /**
     * Applies the cooldown to the calling {@link net.dv8tion.jda.api.entities.User User} local to
     * the {@link net.dv8tion.jda.api.entities.MessageChannel MessageChannel} the command is called
     * in.
     *
     * <p>The key for this is generated in the format
     *
    ...
    /**
     * Applies the cooldown to the {@link net.dv8tion.jda.api.entities.Guild Guild} the command is
     * called in.
     *
     * <p>The key for this is generated in the format
     *
     ...
     * </ul>
     *
     * <p><b>NOTE:</b> This will automatically default back to {@link
     * com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#CHANNEL CooldownScope.CHANNEL} when
     * called in a private channel. This is done in order to prevent internal {@link
     * java.lang.NullPointerException NullPointerException}s from being thrown while generating
     * cooldown keys!
     */
    ...
    /**
     * Applies the cooldown to the calling {@link net.dv8tion.jda.api.entities.User User} local to
     * the {@link net.dv8tion.jda.api.entities.Guild Guild} the command is called in.
     *
     * <p>The key for this is generated in the format
     *
     * <ul>
     ...
     *
     * <p><b>NOTE:</b> This will automatically default back to {@link
     * com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#GLOBAL CooldownScope.GLOBAL} when {@link
     * net.dv8tion.jda.api.JDA#getShardInfo() JDA#getShardInfo()} returns {@code null}. This is done
     * in order to prevent internal {@link java.lang.NullPointerException NullPointerException}s
     * from being thrown while generating cooldown keys!
     */
    SHARD("S:%d", "on this shard"),

    /**
     * Applies the cooldown to the calling {@link net.dv8tion.jda.api.entities.User User} on the
     * Shard the command is called on.
     *
     * <p>The key for this is generated in the format
     *
     * <ul>
     ...
     *
     * <p><b>NOTE:</b> This will automatically default back to {@link
     * com.jagrosh.jmusicbot.jdautils.Command.CooldownScope#USER CooldownScope.USER} when {@link
     * net.dv8tion.jda.api.JDA#getShardInfo() JDA#getShardInfo()} returns {@code null}. This is done
     * in order to prevent internal {@link java.lang.NullPointerException NullPointerException}s
     * from being thrown while generating cooldown keys!
     */
     ...
     * {@link net.dv8tion.jda.api.entities.MessageChannel MessageChannel}s until the cooldown has
     * ended.
     *
