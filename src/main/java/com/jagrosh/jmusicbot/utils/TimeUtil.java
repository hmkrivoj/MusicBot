...
public class TimeUtil {
  ...
  /**
   * Parses a seek time string into milliseconds and determines if it's relative. Supports "colon
   * time" (HH:MM:SS) or "unit time" (1h20m)
   *
  ...
  public static long parseColonTime(String timestamp) {
    ...
    if (timestampSplitArray.length > 3) return -1;
    ...
    return Math.round(
        timeUnitArray[0] * 3600000 + timeUnitArray[1] * 60000 + timeUnitArray[2] * 1000);
  }
  ...
  public static long parseUnitTime(String timestr) {
    timestr =
        timestr.replaceAll("(?i)(\\s|,|and)", "").replaceAll("(?is)(-?\\d+|[a-z]+)", "$1 ").trim();
